-- Phase 2: internal atomic operational transition API.
--
-- Does NOT alter bookings.booking_status CHECK.
-- Does NOT create job offers, proof media, or WhatsApp side effects.
-- CREATE OR REPLACE of sync_booking_operation_from_booking adds a transaction-local
-- suppression flag so this RPC can update bookings without emitting
-- legacy_booking_status_synced. Direct admin/legacy updates still sync.
--
-- Phase 4 will tighten complete_wash (photo proof). Legacy complete_wash
-- remains intentionally permissive until then.
...
BEGIN
  IF pg_catalog.current_setting('washero.operation_command', true) = '1' THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  ELSIF p_command = 'complete_wash' THEN
    -- Phase 4 will require photo proof before completion. Legacy mode is
